    ...
    class SassCompiler
        ...
        def compile(src, dest)
            ...
            if use_compass
                ...
                load_paths = []
                Spider.app_paths.each do |path|
                    load_paths << SassAppImporter.new(path)
                end
                load_paths << Spider.paths[:core_apps]
                ...
                output = SassC::Engine.new(File.read(src_corretto), { style: :compressed, line_comments: true, load_paths: load_paths }).render
