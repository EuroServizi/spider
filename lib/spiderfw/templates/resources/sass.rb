    ...
    class SassCompiler
        ...
        def compile(src, dest)
            ...
            if use_compass
                ...
                #self.class.options[:load_paths] << Spider.paths[:core_apps] la mette già prima..
                Spider.apps.each_pair{ |chiave_app,app|
                    ...
                    self.class.options[:load_paths] << SassAppImporter.new(Spider.apps[chiave_app].path+"/public/scss")
                ...
                #Aggiungo la directory del file src, permette gli import relativi di bootstrap ecc
                self.class.options[:load_paths] << SassAppImporter.new(Pathname.new(src).dirname)
