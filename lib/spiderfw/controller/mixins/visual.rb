...
module Spider; module ControllerMixins
    ...
    module Visual
        ...
        def try_rescue(exc)
            ...
            if exc.respond_to?(:uuid)
                ...
                id_comune = Spider.conf.get('orgs.default.name').gsub(' ','') unless Spider.conf.get('orgs.default.name').blank?
                id_comune ||= Spider.conf.get('ente.nome').gsub(' ','') unless Spider.conf.get('ente.nome').blank?
    ...
    #                self_str = "#<#{frame_self.class}:#{frame_self.object_id}>"
                    ...
                    info += args.map{ |arg|
                        ...
                        elsif (val.is_a?(Symbol) || val.is_a?(Fixnum) || val.is_a?(Integer) || val.is_a?(Float) || val.is_a?(BigDecimal) || val.is_a?(Date) || val.is_a?(Time))
                            val_str = val.inspect
                        end
